...
from utils import (
    ...
    get_upcoming_chores, list_user_chores, send_sms, parse_natural_date
)
...
def handle_sms():
    ...
    if intent == 'add':
        ...
        assignee = User.query.filter(User.name.ilike(assignee_name)).first() if assignee_name else user
        ...
        return dusty_response(
            f"Added chore '{chore_name}' for {assignee.name}. "
            f"Due: {due_date.strftime('%Y-%m-%d') if due_date else 'whenever'}."
        )
